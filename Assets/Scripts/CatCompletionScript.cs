...
public class CatCompletionScript : MonoBehaviour
{
    ...
    void ColorCharacter()
    {
        ...
        if (Box_Coll[18].enabled == false)
            CharacterColor[9].fillAmount = ColliderScaleBrush[18];
        if (Box_Coll[19].enabled == false)
            CharacterColor[9].fillAmount = ColliderScaleBrush[19];

        if (Box_Coll[20].enabled == false)
            CharacterColor[10].fillAmount = ColliderScaleBrush[20];
        if (Box_Coll[21].enabled == false)
            CharacterColor[10].fillAmount = ColliderScaleBrush[21];
    }
